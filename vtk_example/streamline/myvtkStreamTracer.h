...
=========================================================================*/
/**
 * @class   myvtkStreamTracer
 * @brief   Streamline generator
 *
 * myvtkStreamTracer is a filter that integrates a vector field to generate
 * streamlines. The integration is performed using a specified integrator,
 * by default Runge-Kutta2.
 *
 * myvtkStreamTracer produces polylines as the output, with each cell (i.e.,
 ...
 *
 * myvtkStreamTracer supports forward (the default), backward, and combined
 ...
 *
 * myvtkStreamTracer supports integration through any type of dataset. Thus if
...
#ifndef myvtkStreamTracer_h
#define myvtkStreamTracer_h
...
class VTKFILTERSFLOWPATHS_EXPORT myvtkStreamTracer : public vtkPolyDataAlgorithm
{
public:
  vtkTypeMacro(myvtkStreamTracer, vtkPolyDataAlgorithm);
...
private:
  myvtkStreamTracer(const myvtkStreamTracer&) = delete;
  void operator=(const myvtkStreamTracer&) = delete;
};

#endif
// VTK-HeaderTest-Exclude: myvtkStreamTracer.h
