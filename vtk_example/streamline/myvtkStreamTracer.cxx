...
vtkObjectFactoryNewMacro(myvtkStreamTracer);
vtkCxxSetObjectMacro(myvtkStreamTracer, Integrator, vtkInitialValueProblemSolver);
vtkCxxSetObjectMacro(myvtkStreamTracer, InterpolatorPrototype, vtkAbstractInterpolatedVelocityField);

const double myvtkStreamTracer::EPSILON = 1.0E-12;
...
//------------------------------------------------------------------------------
//construct the stream tracer
//this contains all the default parameters
myvtkStreamTracer::myvtkStreamTracer()
{
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::SetSourceConnection(vtkAlgorithmOutput* algOutput)
{
  this->SetInputConnection(1, algOutput);
}

//------------------------------------------------------------------------------
void myvtkStreamTracer::SetSourceData(vtkDataSet* source)
{
  this->SetInputData(1, source);
}

//------------------------------------------------------------------------------
vtkDataSet* myvtkStreamTracer::GetSource()
{
...
//------------------------------------------------------------------------------
int myvtkStreamTracer::GetIntegratorType()
{
  if (!this->Integrator)
  {
    return NONE;
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::SetInterpolatorTypeToDataSetPointLocator()
{
  this->SetInterpolatorType(static_cast<int>(INTERPOLATOR_WITH_DATASET_POINT_LOCATOR));
}

//------------------------------------------------------------------------------
void myvtkStreamTracer::SetInterpolatorTypeToCellLocator()
{
  this->SetInterpolatorType(static_cast<int>(INTERPOLATOR_WITH_CELL_LOCATOR));
}

//------------------------------------------------------------------------------
void myvtkStreamTracer::SetInterpolatorType(int interpType)
{
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::SetIntegratorType(int type)
{
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::SetIntegrationStepUnit(int unit)
{
...
//------------------------------------------------------------------------------
double myvtkStreamTracer::ConvertToLength(double interval, int unit, double cellLength)
{
  double retVal = 0.0;
  if (unit == LENGTH_UNIT)
  {
...
//------------------------------------------------------------------------------
double myvtkStreamTracer::ConvertToLength(
  myvtkStreamTracer::IntervalInformation& interval, double cellLength)
{
  return ConvertToLength(interval.Interval, interval.Unit, cellLength);
}

//------------------------------------------------------------------------------
void myvtkStreamTracer::ConvertIntervals(
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::CalculateVorticity(
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::InitializeSeeds(vtkDataArray*& seeds, vtkIdList*& seedIds,
...
//------------------------------------------------------------------------------
int myvtkStreamTracer::SetupOutput(vtkInformation* inInfo, vtkInformation* outInfo)
{
...
//------------------------------------------------------------------------------
int myvtkStreamTracer::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  std::cout << "---VTK request data---" << std::endl;
  vtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  vtkInformation* outInfo = outputVector->GetInformationObject(0);
  
  //extract the input info
  ...
  if (seeds)
  {
    ...
    if (vectors)
    {
      ...
      //the important step, exeucte the integration
...
//------------------------------------------------------------------------------
int myvtkStreamTracer::CheckInputs(vtkAbstractInterpolatedVelocityField*& func, int* maxCellSize)
{
  if (!this->InputData)
  {
    return VTK_ERROR;
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::Integrate(vtkPointData* input0Data, vtkPolyData* output,
  ...
  for (int currentLine = 0; currentLine < numLines; currentLine++)
  {
    ...
    if (this->ComputeVorticity)
    {
      if (vecType == vtkDataObject::POINT)
      {
        ...
        myvtkStreamTracer::CalculateVorticity(cell, pcoords, cellVectors, vort);
      }
      else
      {
        vort[0] = 0;
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::GenerateNormals(vtkPolyData* output, double* firstNormal, const char* vecName)
{
  // Useful pointers
  vtkDataSetAttributes* outputPD = output->GetPointData();
...
// than Integrate.
double myvtkStreamTracer::SimpleIntegrate(
...
//------------------------------------------------------------------------------
int myvtkStreamTracer::FillInputPortInformation(int port, vtkInformation* info)
{
  if (port == 0)
  {
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::AddCustomTerminationCallback(
...
//------------------------------------------------------------------------------
void myvtkStreamTracer::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Integrator: " << this->Integrator->GetClassName() << endl;
...
//------------------------------------------------------------------------------
vtkExecutive* myvtkStreamTracer::CreateDefaultExecutive()
{
