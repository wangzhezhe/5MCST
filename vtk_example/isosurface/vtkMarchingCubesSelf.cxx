...
struct ComputeGradientWorker {
  ...
  void operator()(ScalarArrayT* scalarsArray, vtkMarchingCubes* self,
                  int dims[3], vtkIncrementalPointLocator* locator,
                  vtkDataArray* newScalars, vtkDataArray* newGradients,
                  vtkDataArray* newNormals, vtkCellArray* newPolys,
                  double* values, vtkIdType numValues) const {
    ...
    for (k = 0; k < (dims[2] - 1); k++) {
      ...
      for (j = 0; j < (dims[1] - 1); j++) {
        ...
        for (i = 0; i < (dims[0] - 1); i++) {
          ...
          // the s here represents the scarlar value at particular points
          // we get value from the scalar array
          // std::cout << "s " << s[0]<< " " << s[1] << " " <<s[2] << " " <<
          // s[3] << s[4] << std::endl; there is no idea of the topology here,
          // then how do we map the point into the topology?
          // do nothing when the s value is out of the bounds
          ...
          std::cout << "debug extent " << extent[0] << "," << extent[1] << ","
          ...
          std::cout << "i" << i << " pts " << pts[0][0] << " " << pts[0][1]
          ...
          if (NeedGradients) {
            vtkMarchingCubesComputePointGradient(i, j, k, scalars, dims,
                                                 sliceSize, gradients[0]);
            vtkMarchingCubesComputePointGradient(i + 1, j, k, scalars, dims,
                                                 sliceSize, gradients[1]);
            vtkMarchingCubesComputePointGradient(i + 1, j + 1, k, scalars, dims,
                                                 sliceSize, gradients[2]);
            vtkMarchingCubesComputePointGradient(i, j + 1, k, scalars, dims,
                                                 sliceSize, gradients[3]);
            vtkMarchingCubesComputePointGradient(i, j, k + 1, scalars, dims,
                                                 sliceSize, gradients[4]);
            vtkMarchingCubesComputePointGradient(i + 1, j, k + 1, scalars, dims,
                                                 sliceSize, gradients[5]);
            vtkMarchingCubesComputePointGradient(i + 1, j + 1, k + 1, scalars,
                                                 dims, sliceSize, gradients[6]);
            vtkMarchingCubesComputePointGradient(i, j + 1, k + 1, scalars, dims,
                                                 sliceSize, gradients[7]);
          }
          for (contNum = 0; contNum < numValues; contNum++) {
            value = values[contNum];
            // Build the case table
            for (ii = 0, index = 0; ii < 8; ii++) {
              // select the ii that make the ii larger then value
              // the idea of the index is that, the digit with the value larger
              // than thresold that digit is the 1, so we use the case_mast to
              // implement this function
              if (s[ii] >= value) {
                index |= CASE_MASK[ii];
                // refer to the paper, there is 256 cases in total which is 2^8
                // std::cout << "debug index "<< index << " ii " << ii <<
                // std::endl;
              }
            }
            ...
            edge = triCase->edges;
            // the value edge here contains all 16 index
            OutPutNum++;
            if(OutPutNum < 3){
              for(int k=0;k<16;k++){
                std::cout << edge[k] << ",";
            ...
            for (; edge[0] > -1; edge += 3) {
              ...
              {
                //the edges stores each pair 
                //vert stors two actual edges of the voxel
                //all values stores in the casetable is 0 to 11
                //it represents which edge has the intersection for triangle
                vert = edges[edge[ii]];
                if (OutPutNum < 3) {
                  std::cout << "ii " << ii << " edge[ii] " << edge[ii]
                            << " vert " << vert[0] << "," << vert[1] << std::endl;
                }
                t = (value - s[vert[0]]) / (s[vert[1]] - s[vert[0]]);
                //get the coordinates of the two vertexes among the edge
                x1 = pts[vert[0]];
                x2 = pts[vert[1]];
...
int vtkMarchingCubes::RequestData(vtkInformation* vtkNotUsed(request),
  ...
  if (inArrayVec) {  // we have been passed an input array
  ...
  estimatedSize =
      static_cast<vtkIdType>(pow(1.0 * dims[0] * dims[1] * dims[2], 0.75));
  ...
  if (!Dispatcher::Execute(
          inScalars, worker, this, dims, this->Locator, newScalars,
          newGradients, newNormals, newPolys, values,
          numContours)) {  // Fallback to slow path for unknown arrays:
...
void vtkMarchingCubes::PrintSelf(ostream& os, vtkIndent indent) {
  ...
  os << indent
     << "Compute Normals: " << (this->ComputeNormals ? "On\n" : "Off\n");
  os << indent
     << "Compute Gradients: " << (this->ComputeGradients ? "On\n" : "Off\n");
  os << indent
     << "Compute Scalars: " << (this->ComputeScalars ? "On\n" : "Off\n");
