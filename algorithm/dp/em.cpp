 ...
 public:
  int lengthOfLIS(vector<int>& nums) {
    ...
    for (int i = 1; i < size; i++) {
      ...
      int R = dp.size();
      // I'm still confused about this process, why the following operation of
      // the binary search can find out the suitable value the length of the dp
      // array is changing for every iteration maybe just consider the last three
      // elements and to see how the following algorithm works
      while (L < R) {
        // find elements
        int mid = (R + L) / 2;
        if (dp[mid] < nums[i]) {
      ...
      // case 2, R is changed, there are case that element is larger than
      // targeted value when L is larger than R, the current value of R is the
      // first value than targeted elements
    ...
    for (int i = 0; i < r; i++) {
      for (int j = 0; j < c; j++) {
        ...
        while (currLen > maxLen) {
          ...
          int k = 0;
          for (k = 0; k < currLen; k++) {
            if (matrix[i - k][j - currLen + 1] == 0 ||
                matrix[i - currLen + 1][j - k] == 0) {
              break;
            }
          }
...
// some test case may beyond the upper bound of the integer, it is better to use
// the double
class Solution {
 public:
  double knightProbability(int N, int K, int r, int c) {
    ...
    vector<vector<int>> dist = {{-1, -2}, {-2, -1}, {-2, 1}, {-1, 2},
                                {1, 2},   {2, 1},   {2, -1}, {1, -2}};
    ...
    for (int s = 0; s < K; s++) {
      ...
      for (int x = 0; x < N; x++) {
        for (int y = 0; y < N; y++) {
          ...
          // std::cout<< s << "x " <<x<<" y "<< y << " v " << dpNew[x][y] <<
          // std::endl;
          valid += dpNew[x][y];
        }
      }
...
class Solution {
 public:
  ...
  int helper(vector<int>& cost, vector<int>& m, int i) {
    ...
    // history
    if (m[i] > 0) return m[i];
    m[i] = min(helper(cost, m, i - 1) + cost[i - 1],
               helper(cost, m, i - 2) + cost[i - 2]);
    return m[i];
  }
};
...
class NumMatrix {
 public:
  ...
  int sumRegion(int row1, int col1, int row2, int col2) {
    ...
    sum = sums[row2 + 1][col2 + 1] - sums[row2 + 1][col1] -
